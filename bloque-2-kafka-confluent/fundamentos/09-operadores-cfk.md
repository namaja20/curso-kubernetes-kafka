# Operadores y CFK (Confluent for Kubernetes)
...
Un **operador de Kubernetes** es un controlador especializado que entiende **un dominio de aplicación** (en este caso, Kafka y el resto de la plataforma Confluent). Se le declara lo que se quiere (*"un cluster Kafka de 3 brokers con TLS y Schema Registry replicado"*) mediante recursos personalizados (CRDs), y el operador genera y mantiene **todos** los objetos Kubernetes necesarios: StatefulSets, Services, ConfigMaps, Secrets, certificados, volúmenes. **CFK** es el operador oficial de Confluent.

## El patrón "operador"

En Kubernetes, un **controlador** es un proceso que mira el cluster, compara estado deseado con real y actúa. Los controladores nativos (Deployment, ReplicaSet, StatefulSet) saben gestionar objetos genéricos.
...
- **CRDs (Custom Resource Definitions)**: extienden la API de Kubernetes con tipos nuevos (p. ej. `Kafka`, `KafkaTopic`, `SchemaRegistry`). Para el cluster son objetos como cualquier otro.
...
> Un Deployment es como un cocinero que sabe hacer un plato genérico. Un operador es un chef especializado que sabe cocinar **paella**: conoce los tiempos del sofrito, cuándo añadir el caldo y cuándo apagar el fuego.
...
Cuando se aplica uno de estos CRs, **CFK** se encarga de:
...
El operador es lo que convierte un YAML de tres pantallas en un cluster Kafka con todo el cableado interno. Sin operador, todo eso habría que escribirlo y mantenerlo a mano.

## Una operación clave: rolling update de los brokers
...
## Topología típica de un cluster Confluent en Kubernetes
...
Eso es lo que aparecerá en `kubectl get pods` al inspeccionar un cluster real. No hay que tenerle miedo a la cantidad: cada pod tiene una función reconocible.
...
- **Integración con K8s.** Métricas vía Prometheus, logs por stdout (a la solución central), networking unificado.
- **Soporte.** En entornos enterprise, Confluent soporta CFK y sus integraciones.

## Limitaciones y cosas a saber

- **No es magia.** Sigue siendo necesario entender qué hace Kafka. Si el cluster pierde quórum, el operador no lo arregla sin ayuda.
...
## Otros operadores que conviene nombrar

Hay otros operadores en el ecosistema:

- **Strimzi** — operador open source para Apache Kafka. Muy popular fuera de entornos Confluent. Si en una organización no hay licencia Confluent, suele ser la opción.
- **Confluent for Kubernetes (CFK)** — el que se usa aquí. Específico de Confluent Platform; incluye los CRDs para todo el ecosistema (Schema Registry, Connect, etc.).
- **Operadores propios o de cloud provider** — menos habituales.

El patrón conceptual (CRDs + operador) es idéntico en todos los casos.
...
4. A partir de ahí ya se pueden aplicar CRs como `Kafka`, `SchemaRegistry`, etc.
...
- **¿Sin CFK no se puede correr Kafka en K8s?** Sí se puede, pero acaba escribiéndose y manteniéndose a mano lo que CFK ya hace. Para producción seria, el operador casi siempre vale la pena.
- **¿Por qué brokers son StatefulSet y no Deployment?** Porque cada broker tiene **identidad** (nombre estable: `kafka-0`, `kafka-1`...) y **volumen propio**. Se vio en el bloque 1.
- **¿El operador toca mis topics?** Solo si se declaran como `KafkaTopic` (CR). Si se crean con `kafka-topics` desde la CLI, son **invisibles** para CFK: opera el cluster, no los topics.
- **¿Se pueden mezclar Strimzi y CFK?** Técnicamente no: cada operador asume que es dueño del cluster que gestiona. En un mismo K8s pueden coexistir si gestionan namespaces distintos, pero **el mismo cluster Kafka solo lo opera un operador**.
- **¿Y la migración de clusters viejos?** Hay procedimientos documentados de Confluent. Quedan fuera del alcance de este texto.

## Recapitulación del bloque

Si los conceptos siguientes están claros, todo lo que viene después es aplicación práctica:
...
El siguiente bloque une lo aprendido en los dos primeros: cómo se ve **Kafka operado dentro de Kubernetes** y cómo se construye un pipeline real Producer → Topic → Consumer sobre ese montaje.
